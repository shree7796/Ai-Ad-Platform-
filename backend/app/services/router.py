...
class ModelRouter:
    ...
    async def get_best_provider(
        ...
        """
        logger.info(f"Routing task '{task_type}' for tier '{user_tier}'. Requested: {requested_provider}")
        ...
        if not valid_models:
            logger.warning(f"No valid models found for task {task_type} and tier {user_tier}. Falling back to Mock.")
            return MockProvider(name="mock_fallback")
        ...
        if requested_provider:
            for m in valid_models:
                if m.provider == requested_provider:
                    ...
                    break
            if not selected:
                logger.warning(f"Requested provider '{requested_provider}' not found or not available for this tier. Falling back to auto-select.")
        
        if not selected:
            # Prioritize quality score (highest) and favor real providers over mock
            # We sort by (not_mock, quality_score, -cost)
            valid_models.sort(
                key=lambda m: (m.provider != "mock", m.quality_score, -m.cost_per_unit),
                reverse=True
            )
            selected = valid_models[0]

        logger.info(f"Selected model: {selected.name} (Provider: {selected.provider})")
